...
<head>
    ...
    <title>@yield('title', 'RecipeHub - Receptes')</title>
        ...
        <div class="max-w-7xl mx-auto px-6 py-4">
            <div class="flex justify-between items-center">
                ...
                <div class="hidden md:flex items-center space-x-6">
                    <a href="{{ route('recipes.index') }}" class="text-gray-700 hover:text-orange-600 font-medium transition duration-200">
                        Receptes
                    </a>
                    
                    <!-- Categories Dropdown -->
                    <div class="relative" x-data="{ open: false }">
                        <button @click="open = !open" class="flex items-center text-gray-700 hover:text-orange-600 font-medium transition duration-200">
                            Kategorijas
                        ...
                        <div x-show="open" @click.away="open = false" 
                            ...
                            @foreach(\App\Models\Category::all() as $category)
                                <a href="{{ route('categories.show', $category) }}" 
                                   class="block px-4 py-2 text-gray-700 hover:bg-gray-50 hover:text-orange-600 {{ $loop->first ? 'rounded-t-lg' : '' }} {{ $loop->last ? 'rounded-b-lg' : '' }}">
                                    {{ $category->name }}
                                </a>
                            @endforeach
                        </div>
                    ...
                    @auth
                        ...
                        <div class="relative" x-data="{ open: false }">
                            ...
                            <div x-show="open" @click.away="open = false"
                                 ...
                                 x-transition:leave-end="transform opacity-0 scale-95"
                                 class="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 z-50">
                                <a href="{{ route('profile.edit') }}" class="block px-4 py-2 text-gray-700 hover:bg-gray-50 hover:text-orange-600 rounded-t-lg">
                                    Rediģēt profilu
                                </a>
                                <a href="{{ route('profile.favorites') }}" class="block px-4 py-2 text-gray-700 hover:bg-gray-50 hover:text-orange-600">
                                    Iemīļotās receptes
                                </a>
                                <a href="{{ route('profile.recipes') }}" class="block px-4 py-2 text-gray-700 hover:bg-gray-50 hover:text-orange-600">
                                    Manas receptes
                                </a>
                                ...
                                <form action="{{ route('logout') }}" method="POST" class="block">
                                    @csrf
                                    <button type="submit" class="w-full text-left px-4 py-2 text-gray-700 hover:bg-gray-50 hover:text-orange-600 rounded-b-lg">
                                        Atslēgties
                                    </button>
                                </form>
                            </div>
            ...
            <div id="mobile-menu" class="md:hidden mt-4 pb-4 border-t border-gray-200 pt-4 hidden">
                ...
                <div class="space-y-2">
                    <a href="{{ route('recipes.index') }}" class="block text-gray-700 hover:text-orange-600 font-medium">
                        Receptes
                    </a>
                    <a href="{{ route('categories.index') }}" class="block text-gray-700 hover:text-orange-600 font-medium">
                        Kategorijas
                    </a>
                    ...
                    @auth
                        ...
                        <a href="{{ route('profile.edit') }}" class="block text-gray-700 hover:text-orange-600 font-medium">
                            Rediģēt profilu
                        </a>
                        <a href="{{ route('profile.favorites') }}" class="block text-gray-700 hover:text-orange-600 font-medium">
                            Iemīļotās receptes
                        </a>
                        <a href="{{ route('profile.recipes') }}" class="block text-gray-700 hover:text-orange-600 font-medium">
                            Manas receptes
                        </a>
                        @if(Auth::user()->is_admin)
                            <a href="{{ route('admin.index') }}" class="block text-purple-700 hover:text-purple-800 font-medium">
                                Admin Dashboard
                            </a>
                        @endif
                        <form action="{{ route('logout') }}" method="POST" class="block">
                            @csrf
                            <button type="submit" class="text-left text-gray-700 hover:text-orange-600 font-medium">
                                Atslēgties
                            </button>
                        </form>
                    @else
                        <a href="{{ route('login') }}" class="block text-gray-700 hover:text-orange-600 font-medium">
                            Pieslēgties
                        </a>
