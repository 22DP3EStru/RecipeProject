...
CMD ["sh", "-c", "php artisan config:clear && php artisan migrate --force && mysql --skip-ssl -h$DB_HOST -P$DB_PORT -u$DB_USERNAME -p$DB_PASSWORD $DB_DATABASE < /var/www/recipeproject.sql && php-fpm -D && nginx -g 'daemon off;'"]
